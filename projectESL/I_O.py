...
def get_gpd_params_from_Hermans2023(cfg,filepath,sites,esl_statistics):
    gpd_params = xr.open_dataset(filepath)
    min_idx = [mindist(x,y,gpd_params.lat.values,gpd_params.lon.values, 0.1)[0] for x,y in zip(sites.lat.values, sites.lon.values)]
...
def get_gum_amax_from_CoDEC(cfg,filepath,sites,esl_statistics):
    
    codec_gum = xr.open_dataset(filepath)
...
def get_coast_rp_return_curves(cfg,filepath,sites,esl_statistics):
    coast_rp_coords = pd.read_pickle(filepath)
